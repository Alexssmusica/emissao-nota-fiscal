import type { Empresa, Geral, NFeBase, SoapInfo, TNFeInfNFeSupl } from '../../../domain/contracts/repos';
...
export async function appendQRCodeXML(
  ...
  xmlAssinado: string,
  empresa: Empresa,
  soapAutorizacao: SoapInfo.Output,
  geral: Geral
): Promise<{ qrCode: TNFeInfNFeSupl; xml: string }> {
  let qrCode: string | null = null;
  const xmlAssinadoObj = await deserializeXml(xmlAssinado, { explicitArray: false });
  const chave = xmlAssinadoObj.NFe.infNFe.$.Id.replace('NFe', '');
  if (documento.ide.tpEmis === '9') {
    const diaEmissao = documento.ide.dhEmi.substring(8, 10);
    const valorTotal = documento.total.ICMSTot.vNF;
    const digestValue = xmlAssinadoObj.NFe.Signature.SignedInfo.Reference.DigestValue;
    ...
    qrCode = gerarQRCodeNFCeOnline(String(soapAutorizacao.urlQRCode), chave, '2', geral.ambiente, empresa.idCSC, empresa.CSC);
  }
  const qrCodeObj = {
    qrCode: '<' + qrCode + '>',
    urlChave: soapAutorizacao.urlChave
  } as TNFeInfNFeSupl;
  const qrCodeXml = serializeXml(qrCodeObj, 'infNFeSupl').replace('>]]>', ']]>').replace('<![CDATA[<', '<![CDATA[');
  return {
    qrCode: qrCodeObj,
