...
import type { Configuracoes, SolicitaEvento, TCodUfIbge } from '../../../domain/contracts/repos';
...
export async function gerarEvento(
  cuf: TCodUfIbge,
  cnpj: string,
  evento: SolicitaEvento,
...
async function transmitirXml(cUf: TCodUfIbge, xml: string, configuracoes: Configuracoes) {
  ...
  return await makeSoapRequest({ xml, empresa, soap, webProxy, cUf });
}
...
function getInfEvento(cuf: TCodUfIbge, cnpj: string, evento: SolicitaEvento, configuracoes: Configuracoes) {
  ...
  if (_ID.length < 54) throw new Error('ID de Evento inválido');
